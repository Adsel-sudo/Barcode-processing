...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Base output directory for each run")
    parser.add_argument("--report", type=Path, default=None, help="Report filename (.csv or .xlsx), saved under run report dir")
    ...
    parser.add_argument(
        ...
        default=True,
        help="Delete source PDF after full successful pipeline completion",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    print(f"run output dir: {result.run_output_dir}")
    print(f"image dir: {result.image_dir}")
    print(f"report: {result.report_path}")
    print("all current run outputs are saved under run output dir")
    ...
    print(f"source pdf deleted: {result.source_pdf_deleted}")
    if result.source_pdf_delete_error:
        print(f"[WARN] failed to delete source pdf: {result.source_pdf_delete_error}")
    print("=" * 88)
